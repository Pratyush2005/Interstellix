...
const OpeningAnimation: React.FC<OpeningAnimationProps> = ({ onComplete }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black flex items-center justify-center overflow-hidden">
      ...
      {/* Shooting stars during intro */}
      {stage >= 1 && (
        <>
          <div className="absolute top-1/4 left-1/4 w-2 h-2 bg-white rounded-full animate-ping"></div>
          <div className="absolute top-1/3 right-1/3 w-1 h-1 bg-cyan-400 rounded-full animate-pulse"></div>
          <div className="absolute bottom-1/3 left-1/3 w-1.5 h-1.5 bg-blue-400 rounded-full animate-bounce"></div>
        </>
      )}
      ...
      <div className="text-center z-10">
        ...
        {stage >= 2 && (
          <div className={`transition-all duration-1000 ${stage >= 2 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            <p className="text-cyan-300 text-xl md:text-2xl font-light tracking-wide">
        ...
        {stage >= 3 && (
          <div className={`mt-8 transition-all duration-1000 ${stage >= 3 ? 'opacity-100' : 'opacity-0'}`}>
