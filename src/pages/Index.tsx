...
const Index = () => {
  ...
  const handleSearch = useCallback(
    (query: string) => {
      ...
      handleUserInteraction(); // Reset idle prompt on search
    },
    []
  );

  const handleSearchClose = useCallback(() => {
    setSearchQuery(""); // Clear search query to close Search component
    handleUserInteraction(); // Reset idle prompt on close
  }, []);

 const [showFunFacts, setShowFunFacts] = useState(true); // Sidebar open by default

   const handleToggleFunFacts = () => {
  ...
  const [astronautBubbleMessage, setAstronautBubbleMessage] = useState<
    string | null
  >(null);
  ...
  const cyclingHintMessages = [
    // Messages to cycle through after the first one
    "Still there, comrade?",
  ...
  const startCyclingHints = useCallback(() => {
    ...
    hintCycleTimerRef.current = setTimeout(() => {
      currentHintMessageIndex.current =
        (currentHintMessageIndex.current + 1) % cyclingHintMessages.length;
      setAstronautBubbleMessage(
        cyclingHintMessages[currentHintMessageIndex.current]
      );
  ...
  const resetIdlePrompt = useCallback(() => {
    ...
    idlePromptTimerRef.current = setTimeout(() => {
      const randomMessage =
        cyclingHintMessages[
          Math.floor(Math.random() * cyclingHintMessages.length)
        ];
  ...
  useEffect(() => {
    ...
    if (shouldShowBubble) {
      if (hintPhase === "initial") {
        ...
        if (
          astronautBubbleMessage === null ||
          astronautBubbleMessage === firstHintMessage ||
          !cyclingHintMessages.includes(astronautBubbleMessage)
        ) {
          setAstronautBubbleMessage(
            cyclingHintMessages[currentHintMessageIndex.current]
          );
      ...
      if (initialHintDisplayTimerRef.current)
        clearTimeout(initialHintDisplayTimerRef.current); // Clear initial hint timer if hidden
      ...
      if (!isChatOpen && !isAstronautHintDelayed) {
        // Only reset to initial if not hidden by chat or its delay
    ...
    return () => {
      ...
      if (chatCloseDelayTimerRef.current)
        clearTimeout(chatCloseDelayTimerRef.current); // NEW cleanup
    };
  }, [
    showOpening,
    showSystemFailure,
  ...
  useEffect(() => {
    if (humAudioRef.current) {
      ...
      humAudioRef.current
        ...
        .catch((e) =>
          console.log(
            "Background hum audio play failed (user interaction might be needed):",
            e
          )
        );
    }
  ...
  useEffect(() => {
    ...
    handleActivity(); // Show initially, then hide after timeout
  ...
  const toggleChatWindow = () => {
    ...
    if (newOpenState) {
      // If chat is being opened by clicking the astronaut button
      ...
      // If chat is being closed by clicking the astronaut button
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden">
      ...
      {showSystemFailure && (
        <SystemFailureTerminal
          onRecoveryComplete={handleSystemRecoveryComplete}
          isCrashScenario={isCrashScenarioActive}
        />
      )}

      <div
        className={`transition-opacity duration-1000 ${
          showOpening || showSystemFailure ? "opacity-0" : "opacity-100"
        }`}
      >
        <StarField />
        <Navigation
          ...
          onToggleFunFacts={handleToggleFunFacts} // New prop
        />
        ...
        <main className="relative z-10">
          ...
          {/* 🔥 ADD THIS SEARCH COMPONENT HERE */}
          {searchQuery && (
            <Search
              query={searchQuery}
        ...
        {showFunFacts && <FunFactsSidebar onClose={() => setShowFunFacts(false)} />}
